...
do*. Cairn is that memory: a plain-text registry, committed next to your code, in
which every feature is one **stone** carrying the user's own words — and one
deterministic Playwright **proof** that the promise is still kept.
...
> checkout: `pnpm add -D file:../cairn/packages/cli`, or run the built binary
> directly with `node path/to/cairn/packages/cli/dist/bin.js`.
...
| [`.claude/skills`](.claude/skills) | The three agent skills: [`cairn-mason`](.claude/skills/cairn-mason/SKILL.md), [`cairn-warden`](.claude/skills/cairn-warden/SKILL.md), [`cairn-triage`](.claude/skills/cairn-triage/SKILL.md). They belong in the *target* project; they live here to be versioned with the CLI they call. |
| [`docs/orchestration.md`](docs/orchestration.md) | The loop, the permission model that makes both blindnesses real (deny rules, subagent tools, OS file modes), the integrity hook. |
| [`docs/ci.md`](docs/ci.md) | The red-pipeline protocol, written to be read in anger at 18:40. Exit codes, flakiness policy, the triage runbook. |
| [`docs/first-session.md`](docs/first-session.md) | One feature from a sentence to a proven stone. |
